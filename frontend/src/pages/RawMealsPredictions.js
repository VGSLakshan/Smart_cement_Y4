...
export default function RawMealsPredictions({ predictions, onDelete }) {
  if (predictions.length === 0) {
    return (
      <p className="text-gray-500 text-center mt-10">No predictions yet</p>
    );
  }
        ...
        <div key={index} className="bg-white rounded-xl shadow-md p-4">
          ...
          <p
            className={`text-xs mb-2 font-semibold ${
              item.counts.microscopic ? "text-green-600" : "text-red-600"
            }`}
          >
            {item.counts.microscopic ? "Microscopic Image" : "Not Microscopic"}
          </p>

          {!item.counts.microscopic ? (
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-red-100 rounded p-2">
                ...
                <p className="font-bold text-red-700">{item.counts.dark_red}</p>
              </div>
              ...
              <div className="bg-gray-200 rounded p-2">
                ...
                <p className="font-bold">{item.counts.white}</p>
              </div>
            </div>
          )}
        </div>
