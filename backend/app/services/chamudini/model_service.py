...
class CementClassifierModel:
    ...
    def _load_model(self):
        """Load the trained Keras model with integrity checks"""
        
        try:
            if not self.model_path.exists():
                raise FileNotFoundError(f"Model file not found: {self.model_path}")
            
            # Check if the file is empty (common cause of 'line 1 column 1' error)
            if os.path.getsize(self.model_path) == 0:
                raise ValueError(f"Model file at {self.model_path} is empty (0 bytes).")

            logger.info(f"Loading model from: {self.model_path}")
            
            # Use compile=False to avoid issues with custom metrics/optimizers
            # during simple inference
            ...
            # Setting model to None ensures is_loaded() correctly returns False
            self.model = None 
